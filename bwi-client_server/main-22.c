...
MAIN_BEGIN("bwi-client_server-22", "stderr", NULL)
{
  ...
  fork_proc(cpu_hog_cbs_proc) {
    exec_proc("./cpu_hog_cbs", make_opt("e", "10"), make_opt("b", "0"),
              make_opt("q", "6"), make_opt("t", "30"), make_opt("z", ""));
  }
  sleep(1);
  ...
  fork_proc(client_proc) {
    exec_proc("./client", make_opt("1", "5"), make_opt("2", "9"),
              ...
              make_opt("s", "subexperiment_22.bin"), make_opt("v", server_pid),
  ...
  rc = kill_proc(cpu_hog_cbs_proc, SIGUSR1) ? EXIT_FAILURE : rc;

  sleep(60);

  rc = kill_proc(client_proc, SIGTERM) ? EXIT_FAILURE : rc;
  rc = kill_proc(cpu_hog_cbs_proc, SIGTERM) ? EXIT_FAILURE : rc;
  sleep(1);
  ...
  rc = wait_proc(cpu_hog_cbs_proc) ? EXIT_FAILURE : rc;
...
error:
  ...
  kill_and_wait_proc(cpu_hog_cbs_proc, SIGKILL);
